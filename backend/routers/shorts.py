...
async def interact_short(lesson_id: int, interaction: dict, db = Depends(get_db)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM Lms_short_lessons WHERE id = %s", (lesson_id,))
        ...
        if action == "like":
            cursor.execute("UPDATE Lms_short_lessons SET likes = likes + 1 WHERE id = %s", (lesson_id,))
        elif action == "comment":
            cursor.execute("UPDATE Lms_short_lessons SET comments = comments + 1 WHERE id = %s", (lesson_id,))
        
        db.commit()
        
        cursor.execute("SELECT likes, comments FROM Lms_short_lessons WHERE id = %s", (lesson_id,))
