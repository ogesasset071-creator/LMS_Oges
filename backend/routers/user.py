...
async def get_me(current_user: User = Depends(get_current_user)):
    return {
        ...
        "pp": current_user.pp or 0,
        "total_minutes": current_user.total_minutes or 0,
        "bio": current_user.bio or "",
        "avatar": current_user.avatar or "",
        "category": current_user.category or "",
...
async def list_admins(db = Depends(get_db)):
    with db.cursor() as cursor:
        cursor.execute("SELECT id, full_name, bio, avatar, email FROM users WHERE role = 'admin'")
        users = cursor.fetchall()
        
    return [{
        "id": u['id'],
        "full_name": u['full_name'],
        "bio": u['bio'] or "Premium Admin",
        "avatar": u['avatar'] or "",
        "email": u['email']
    } for u in users]
...
async def get_admin_stats(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    with db.cursor() as cursor:
        ...
        cursor.execute("SELECT id FROM courses WHERE tutor_id = %s", (current_user.id,))
        ...
        sql = """SELECT COUNT(DISTINCT p.user_id) as total_students 
                 FROM progress p 
                 JOIN chapters c ON p.lesson_id = c.id 
...
@router.put("/user/profile", response_model=Token)
async def update_profile(req: UserUpdate, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    bio = req.bio if req.bio is not None else current_user.bio
    full_name = req.full_name if req.full_name is not None else current_user.full_name
    avatar = req.avatar if req.avatar is not None else current_user.avatar
    
    with db.cursor() as cursor:
        cursor.execute("UPDATE users SET full_name = %s, bio = %s, avatar = %s WHERE id = %s", (full_name, bio, avatar, current_user.id))
        db.commit()
...
async def get_my_courses(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        ...
        cursor.execute("SELECT lesson_id FROM progress WHERE user_id = %s", (current_user.id,))
        ...
        sql = """SELECT DISTINCT c.id FROM courses c 
                 LEFT JOIN units u ON c.id = u.course_id 
                 LEFT JOIN chapters ch ON (c.id = ch.course_id OR u.id = ch.unit_id) 
                 WHERE ch.id IN %s"""
        ...
        sql = """SELECT c.*, u.full_name as tutor_name 
                 FROM courses c 
        ...
        for c in courses_data:
            ...
            sql = """SELECT id FROM chapters 
                     WHERE course_id = %s 
                     OR unit_id IN (SELECT id FROM units WHERE course_id = %s)"""
            cursor.execute(sql, (c['id'], c['id']))
            ch_ids = [ch['id'] for ch in cursor.fetchall()]
            
            # User progress count
            sql = "SELECT COUNT(*) as done FROM progress WHERE user_id = %s AND completed = TRUE AND lesson_id IN %s"
...
@router.post("/user/progress", response_model=Token)
async def update_progress(lesson_id: int, background_tasks: BackgroundTasks, completed: bool = False, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        ...
        cursor.execute("SELECT id FROM progress WHERE user_id = %s LIMIT 1", (current_user.id,))
        first_progress = cursor.fetchone() is None
        
        cursor.execute("SELECT * FROM progress WHERE user_id = %s AND lesson_id = %s", (current_user.id, lesson_id))
        progress = cursor.fetchone()
        
        if not progress:
            cursor.execute("INSERT INTO progress (user_id, lesson_id, completed, timestamp) VALUES (%s, %s, %s, %s)", 
                           (current_user.id, lesson_id, completed, datetime.utcnow()))
            
            if first_progress:
                cursor.execute("SELECT course_id FROM chapters WHERE id = %s", (lesson_id,))
                chapter = cursor.fetchone()
                if chapter and chapter['course_id']:
                    cursor.execute("SELECT title FROM courses WHERE id = %s", (chapter['course_id'],))
                    course = cursor.fetchone()
                    if course:
                        from mail_utils import notify_course_started
                        background_tasks.add_task(notify_course_started, current_user.email, course['title'])
        else:
            if not progress['completed'] and completed:
                cursor.execute("UPDATE progress SET completed = TRUE, timestamp = %s WHERE id = %s", (datetime.utcnow(), progress['id']))
            else:
                cursor.execute("UPDATE progress SET timestamp = %s WHERE id = %s", (datetime.utcnow(), progress['id']))
        
        xp_gain = 20 if completed else 2
        pp_gain = 60
        if completed:
            cursor.execute("SELECT pp_reward FROM chapters WHERE id = %s", (lesson_id,))
            ch = cursor.fetchone()
            pp_gain = ch['pp_reward'] if ch and ch.get('pp_reward') else 100
        
        cursor.execute("UPDATE users SET xp = xp + %s, pp = pp + %s, total_minutes = total_minutes + 1 WHERE id = %s", 
                       (xp_gain, pp_gain, current_user.id))
        db.commit()

        # Update local user object for response
        current_user.xp += xp_gain
        current_user.pp += pp_gain
        current_user.total_minutes += 1
...
@router.post("/user/courses/{course_id}/complete_all", response_model=Token)
async def complete_whole_course(course_id: int, background_tasks: BackgroundTasks, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        ...
        for ch_id in all_ch_ids:
            cursor.execute("SELECT id FROM progress WHERE user_id = %s AND lesson_id = %s", (current_user.id, ch_id))
            prog = cursor.fetchone()
            if not prog:
                cursor.execute("INSERT INTO progress (user_id, lesson_id, completed, timestamp) VALUES (%s, %s, TRUE, %s)", 
                               (current_user.id, ch_id, datetime.utcnow()))
            else:
                cursor.execute("UPDATE progress SET completed = TRUE, timestamp = %s WHERE id = %s", (datetime.utcnow(), prog['id']))
        
        cursor.execute("UPDATE users SET xp = xp + 100 WHERE id = %s", (current_user.id,))
        current_user.xp += 100
        
        cursor.execute("SELECT c.title, u.email as tutor_email FROM courses c LEFT JOIN users u ON c.tutor_id = u.id WHERE c.id = %s", (course_id,))
        course = cursor.fetchone()
        if course:
            from mail_utils import notify_course_completed, notify_student_completion_to_admin
            background_tasks.add_task(notify_course_completed, current_user.email, course['title'])
            if course['tutor_email']:
                 background_tasks.add_task(notify_student_completion_to_admin, course['tutor_email'], current_user.full_name, course['title'])
...
async def get_leaderboard(db = Depends(get_db)):
    with db.cursor() as cursor:
        cursor.execute("SELECT id, full_name, email, xp, pp, streak, avatar FROM users ORDER BY xp DESC LIMIT 20")
        users = cursor.fetchall()
...
async def get_dashboard_stats(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM progress WHERE user_id = %s", (current_user.id,))
        total_progress = cursor.fetchall()
        
        progress_lessons = [p['lesson_id'] for p in total_progress]
        unique_course_ids = []
        if progress_lessons:
            sql = "SELECT DISTINCT course_id FROM chapters WHERE id IN %s"
        ...
        for c_id in unique_course_ids:
            cursor.execute("SELECT id FROM chapters WHERE course_id = %s", (c_id,))
        ...
        if unique_course_ids:
            ...
            for c in recent_courses_data:
                cursor.execute("SELECT COUNT(*) as cnt FROM chapters WHERE course_id = %s", (c['id'],))
                total_chapters = cursor.fetchone()['cnt']
                
                sql = "SELECT COUNT(*) as cnt FROM progress WHERE user_id = %s AND completed = TRUE AND lesson_id IN (SELECT id FROM chapters WHERE course_id = %s)"
        ...
        cursor.execute("SELECT COUNT(*) as rank_above FROM users WHERE xp > %s", (current_user.xp,))
        ...
        for i in range(6, -1, -1):
            ...
            activity_score = (count * 20) + (current_user.streak * 5 if i < (current_user.streak or 0) else 0)
            chart_data.append({"name": day_str, "activity": activity_score})

        radar_data = []
        if unique_course_ids:
            cursor.execute("SELECT category, COUNT(*) as cnt FROM courses WHERE id IN %s GROUP BY category", (tuple(unique_course_ids),))
...
async def get_admin_dashboard_stats(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    with db.cursor() as cursor:
        cursor.execute("SELECT COUNT(*) as cnt FROM courses WHERE tutor_id = %s", (current_user.id,))
        course_count = cursor.fetchone()['cnt']
        
        cursor.execute("SELECT COUNT(*) as cnt FROM assignments WHERE creator_id = %s", (current_user.id,))
        assign_count = cursor.fetchone()['cnt']
        
        sql = "SELECT COUNT(*) as cnt FROM user_assignments ua JOIN assignments a ON ua.assignment_id = a.id WHERE a.creator_id = %s"
        cursor.execute(sql, (current_user.id,))
        subs_count = cursor.fetchone()['cnt']
        
        sql = """SELECT COUNT(DISTINCT p.user_id) as cnt 
                 FROM progress p 
                 JOIN chapters ch ON p.lesson_id = ch.id 
        ...
        for i in range(7):
            ...
            sql = """SELECT COUNT(*) as cnt FROM user_assignments ua 
...
async def get_assignments(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT COUNT(*) as cnt FROM progress WHERE user_id = %s AND completed = TRUE", (current_user.id,))
        ...
        if cursor.fetchone()['cnt'] == 0:
            ...
            for track in professional_tracks:
                cursor.execute("INSERT INTO assignments (role, category, level, title, description, reward_badge, type) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                               ("learner", track, "Beginner", f"{track}: Foundation Quiz", f"Basic knowledge of {track}.", f"{track} Rookie 🥉", "quiz"))
                a_id = cursor.lastrowid
                cursor.execute("INSERT INTO assignment_questions (assignment_id, question_text, options, question_type) VALUES (%s, %s, %s, %s)",
                               (a_id, f"What is the primary goal of {track}?", "Scale,AI,Data,UI", "mcq"))
            db.commit()

        user_cat = current_user.category or "General"
        sql = "SELECT * FROM assignments WHERE role = 'learner' AND (category = %s OR category = 'General')"
        cursor.execute(sql, (user_cat,))
        assigns_data = cursor.fetchall()
        
        cursor.execute("SELECT assignment_id FROM user_assignments WHERE user_id = %s AND status = 'completed'", (current_user.id,))
        user_done_ids = [ua['assignment_id'] for ua in cursor.fetchall()]
        
        res = []
        for a in assigns_data:
            cursor.execute("SELECT * FROM assignment_questions WHERE assignment_id = %s", (a['id'],))
...
async def get_assignment_detail(id: int, db = Depends(get_db)):
    with db.cursor() as cursor:
        ...
        cursor.execute("SELECT * FROM assignment_questions WHERE assignment_id = %s", (id,))
...
async def complete_assignment(id: int, background_tasks: BackgroundTasks, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM user_assignments WHERE user_id = %s AND assignment_id = %s", (current_user.id, id))
        ...
        cursor.execute("INSERT INTO user_assignments (user_id, assignment_id, status, timestamp) VALUES (%s, %s, 'completed', %s)", 
                       (current_user.id, id, datetime.utcnow()))
        
        pp_inc = assignment['pp_reward'] or 50
        cursor.execute("UPDATE users SET pp = pp + %s WHERE id = %s", (pp_inc, current_user.id))
        
        current_badges = (current_user.badges or "").split(",")
        if assignment['reward_badge'] not in current_badges:
            new_badges = f"{current_user.badges},{assignment['reward_badge']}" if current_user.badges else assignment['reward_badge']
            cursor.execute("UPDATE users SET badges = %s WHERE id = %s", (new_badges, current_user.id))
    ...
    if assignment['type'] == "quiz":
        background_tasks.add_task(notify_quiz_completed, current_user.email, assignment['title'])
    else:
        background_tasks.add_task(notify_assignment_completed, current_user.email, assignment['title'])
        
    if assignment.get('creator_id'):
        with db.cursor() as cursor:
            cursor.execute("SELECT email FROM users WHERE id = %s", (assignment['creator_id'],))
            creator = cursor.fetchone()
            if creator:
                background_tasks.add_task(notify_student_completion_to_admin, creator['email'], current_user.full_name, assignment['title'])
...
async def create_new_assignment(req: AssignmentCreate, background_tasks: BackgroundTasks, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    with db.cursor() as cursor:
        if req.courseId:
            cursor.execute("SELECT category, level FROM courses WHERE id = %s", (req.courseId,))
            c = cursor.fetchone()
            if c: cat, lvl = c['category'], c['level']

        cursor.execute("""INSERT INTO assignments (title, description, reward_badge, role, creator_id, category, level, pp_reward, type) 
        ...
        for q in req.questions:
            cursor.execute("""INSERT INTO assignment_questions (assignment_id, question_text, options, correct_options, question_type, correct_answer_text) 
                              VALUES (%s, %s, %s, %s, %s, %s)""",
                           (a_id, q.question, ",".join(q.options) if q.options else "", q.correct_options or "0", q.question_type or "mcq", q.correct_answer_text))
        db.commit()
        
        cursor.execute("SELECT * FROM assignments WHERE id = %s", (a_id,))
        new_a = cursor.fetchone()
        cursor.execute("SELECT * FROM assignment_questions WHERE assignment_id = %s", (a_id,))
        new_a['questions'] = cursor.fetchall()

    from mail_utils import notify_new_assignment, notify_new_quiz
    if new_a['type'] == "quiz":
        background_tasks.add_task(notify_new_quiz, current_user.email, new_a['title'])
    else:
        background_tasks.add_task(notify_new_assignment, current_user.email, new_a['title'])
...
async def delete_assignment(id: int, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    with db.cursor() as cursor:
        cursor.execute("SELECT creator_id FROM assignments WHERE id = %s", (id,))
        a = cursor.fetchone()
        if not a: raise HTTPException(status_code=404, detail="Not found")
        if a['creator_id'] != current_user.id and current_user.role != "admin": raise HTTPException(status_code=403, detail="Not authorized")
        cursor.execute("DELETE FROM assignments WHERE id = %s", (id,))
        db.commit()
    return {"status": "success"}

@router.get("/user/educator/assignments", response_model=List[AssignmentResponse])
async def get_my_uploaded_assignments(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role not in ["educator", "admin"]: raise HTTPException(status_code=403, detail="Permission denied")
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM assignments WHERE creator_id = %s", (current_user.id,))
...
async def get_all_submissions(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role not in ["educator", "admin"]: raise HTTPException(status_code=403, detail="Permission denied")
    with db.cursor() as cursor:
        sql = """SELECT ua.*, u.full_name as student_name, u.email as student_email, a.title, a.type, a.reward_badge 
                 FROM user_assignments ua 
...
async def get_admin_course_completions(db = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    res = []
    with db.cursor() as cursor:
        cursor.execute("SELECT id, title FROM courses WHERE tutor_id = %s", (current_user.id,))
        my_courses = cursor.fetchall()
        for c in my_courses:
            cursor.execute("SELECT id FROM chapters WHERE course_id = %s", (c['id'],))
            ch_ids = [ch['id'] for ch in cursor.fetchall()]
            ...
            for u in cursor.fetchall():
                res.append({"course_title": c['title'], "student_name": u['full_name'], "student_email": u['email'], "timestamp": datetime.utcnow().isoformat()})
    return res
