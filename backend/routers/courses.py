...
async def create_course(course_data: CourseCreate, background_tasks: BackgroundTasks, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != "admin" and current_user.role != "educator":
        raise HTTPException(status_code=403, detail="Only admins/educators can upload courses")

    with db.cursor() as cursor:
        cursor.execute("""INSERT INTO courses (tutor_id, title, description, category, price, thumbnail, is_approved) 
        ...
        if course_data.units:
            for ui, unit_data in enumerate(course_data.units):
                cursor.execute("INSERT INTO units (course_id, title, order_num) VALUES (%s, %s, %s)", 
                ...
                for ci, chap in enumerate(unit_data.chapters):
                    cursor.execute("""INSERT INTO chapters (course_id, unit_id, title, video_url, description, content_type, order_num, pp_reward) 
                    ...
                    for si, sec in enumerate(chap.sections):
                        cursor.execute("INSERT INTO chapter_sections (chapter_id, heading, description, order_num) VALUES (%s, %s, %s, %s)",
                                       (chapter_id, sec.heading, sec.description, sec.order_num if sec.order_num > 0 else si + 1))
                    
                    for res in (chap.resources or []):
                        cursor.execute("INSERT INTO resources (course_id, chapter_id, title, file_type, file_url) VALUES (%s, %s, %s, %s, %s)",
                                       (course_id, chapter_id, res.title, res.file_type, res.file_url))
            
            for res in (course_data.resources or []):
                cursor.execute("INSERT INTO resources (course_id, title, file_type, file_url) VALUES (%s, %s, %s, %s)",
                               (course_id, res.title, res.file_type, res.file_url))
        else:
            for i, chap in enumerate(course_data.chapters):
                cursor.execute("INSERT INTO chapters (course_id, title, video_url, order_num) VALUES (%s, %s, %s, %s)",
                               (course_id, chap.title, chap.video_url, chap.order_num if chap.order_num > 0 else i + 1))
        
        db.commit()
    
    from mail_utils import notify_new_course
    background_tasks.add_task(notify_new_course, current_user.email, course_data.title)
...
async def get_courses(page: int = 1, limit: int = 12, category: str = "All", db = Depends(get_db)):
    with db.cursor() as cursor:
        ...
        cursor.execute(f"SELECT COUNT(*) as total FROM courses {where_clause}", tuple(params))
        total = cursor.fetchone()['total']
        
        offset = (page - 1) * limit
        sql = f"""SELECT c.*, u.full_name as tutor_name 
                 FROM courses c 
...
async def get_course(course_id: int, db = Depends(get_db)):
    with db.cursor() as cursor:
        cursor.execute("SELECT c.*, u.full_name as tutor_name FROM courses c LEFT JOIN users u ON c.tutor_id = u.id WHERE c.id = %s", (course_id,))
        ...
        # Units
        cursor.execute("SELECT * FROM units WHERE course_id = %s ORDER BY order_num", (course_id,))
        units = cursor.fetchall()
        for unit in units:
            # Chapters for unit
            cursor.execute("SELECT * FROM chapters WHERE unit_id = %s ORDER BY order_num", (unit['id'],))
            chapters = cursor.fetchall()
            for chap in chapters:
                # Sections
                cursor.execute("SELECT * FROM chapter_sections WHERE chapter_id = %s ORDER BY order_num", (chap['id'],))
                chap['sections'] = cursor.fetchall()
                # Resources
                cursor.execute("SELECT * FROM resources WHERE chapter_id = %s", (chap['id'],))
                chap['resources'] = cursor.fetchall()
            unit['chapters'] = chapters
        course['units'] = units
        
        # Direct Chapters (Legacy)
        cursor.execute("SELECT * FROM chapters WHERE course_id = %s AND unit_id IS NULL ORDER BY order_num", (course_id,))
        course['chapters'] = cursor.fetchall()
        
        # Course level resources
        cursor.execute("SELECT * FROM resources WHERE course_id = %s AND chapter_id IS NULL", (course_id,))
    ...
    with db.cursor() as cursor:
        ...
        if not lessons:
            ...
            for l in default_lessons:
                cursor.execute("""INSERT INTO short_lessons (title, description, category, instructor, likes, comments, video_url, has_quiz, quiz_quest, quiz_options) 
...
async def interact_lesson(id: int, req: InteractRequest, db = Depends(get_db)):
    with db.cursor() as cursor:
        if req.action == "like":
            cursor.execute("UPDATE short_lessons SET likes = likes + 1 WHERE id = %s", (id,))
        elif req.action == "comment":
            cursor.execute("UPDATE short_lessons SET comments = comments + 1 WHERE id = %s", (id,))
        db.commit()
        cursor.execute("SELECT * FROM short_lessons WHERE id = %s", (id,))
...
async def update_course(course_id: int, course_data: CourseCreate, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT tutor_id FROM courses WHERE id = %s", (course_id,))
        course = cursor.fetchone()
        if not course:
            raise HTTPException(status_code=404, detail="Course not found")
        if course['tutor_id'] != current_user.id and current_user.role != "admin":
            raise HTTPException(status_code=403, detail="Not authorized")
        
        cursor.execute("UPDATE courses SET title = %s, description = %s, category = %s, price = %s, thumbnail = %s WHERE id = %s",
        ...
        cursor.execute("DELETE FROM units WHERE course_id = %s", (course_id,))
...
async def get_announcements(course_id: int, db = Depends(get_db)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM announcements WHERE course_id = %s ORDER BY timestamp DESC", (course_id,))
...
async def create_announcement(course_id: int, req: AnnouncementCreate, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        ...
        cursor.execute("INSERT INTO announcements (course_id, content, timestamp) VALUES (%s, %s, %s)", (course_id, req.content, datetime.utcnow()))
        db.commit()
        cursor.execute("SELECT * FROM announcements WHERE id = %s", (cursor.lastrowid,))
...
async def get_notes(course_id: int, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM notes WHERE course_id = %s AND user_id = %s", (course_id, current_user.id))
        return cursor.fetchone()
...
async def save_notes(course_id: int, req: NoteCreate, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT id FROM notes WHERE course_id = %s AND user_id = %s", (course_id, current_user.id))
        existing = cursor.fetchone()
        if existing:
            cursor.execute("UPDATE notes SET content = %s WHERE id = %s", (req.content, existing['id']))
        else:
            cursor.execute("INSERT INTO notes (user_id, course_id, content) VALUES (%s, %s, %s)", (current_user.id, course_id, req.content))
        db.commit()
...
async def get_personal_resources(course_id: int, db = Depends(get_db), current_user: User = Depends(get_current_user)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM personal_note_files WHERE course_id = %s AND user_id = %s", (course_id, current_user.id))
        return cursor.fetchall()
...
async def upload_personal_resource(course_id: int, title: str, file: UploadFile = File(...), db = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    with db.cursor() as cursor:
        cursor.execute("INSERT INTO personal_note_files (user_id, course_id, file_title, file_url) VALUES (%s, %s, %s, %s)", 
                       (current_user.id, course_id, title, file_url))
        db.commit()
        cursor.execute("SELECT * FROM personal_note_files WHERE id = %s", (cursor.lastrowid,))
        return cursor.fetchone()
